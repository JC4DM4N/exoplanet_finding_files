...
data_model_kep_b = np.genfromtxt('RV_radial_velocities_b_full.dat',skip_header=1)

data_model_kep_c = np.genfromtxt('RV_radial_velocities_c_full.dat',skip_header=1)

data_model_gp = np.genfromtxt('RV_gp_quasiperiodic_full.dat',skip_header=1)

plt.plot(data_model_gp[:,0],data_model_gp[:,3]+data_model_kep_b[:,3]+data_model_kep_c[:,3],zorder=1)

plt.fill_between(data_model_gp[:,0],(data_model_gp[:,3]+data_model_kep_b[:,3]+data_model_kep_c[:,3])-data_model_gp[:,4],
                 (data_model_gp[:,3]+data_model_kep_b[:,3]+data_model_kep_c[:,3])+data_model_gp[:,4],alpha=0.5)
...
plt.savefig('RV_phase_GP_TOI1778.png')
plt.show()
